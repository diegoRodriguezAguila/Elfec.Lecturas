...
public class SuperListview extends BaseSuperAbsListview {
	...
	protected void initAttrs(AttributeSet attrs) {
		...
		try {
			mSuperListViewMainLayout = a.getResourceId(
					R.styleable.superlistview_superlv_mainLayoutID,
	...
	protected void initAbsListView(View v) {
		...
		if (listView instanceof ListView)
			...
			throw new IllegalArgumentException(
					"SuperListView works with a List!");
		...
		if (mList != null) {
			...
			if (mDivider != 0) {
				getList().setDivider(
						ContextCompat.getDrawable(getContext(), mDivider));
			}
			...
			if (mPadding != -1.0f) {
				...
				mList.setPadding(mPaddingLeft, mPaddingTop, mPaddingRight,
						mPaddingBottom);
			}
	...
	public void setupSwipeToDismiss(
			final SwipeDismissListViewTouchListener.DismissCallbacks listener,
					...
					public void onDismiss(ListView listView,
						...
						if (autoRemove) {
							for (int position : reverseSortedPositions) {
								...
								((ArrayAdapter) mList.getAdapter())
										.remove(mList.getAdapter().getItem(
												position));
							}
							((ArrayAdapter) mList.getAdapter())
