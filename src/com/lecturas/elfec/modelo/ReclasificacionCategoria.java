...
/**
 * Almacena la información sobre las reclasificaciones de categorias, de la tabla 
 * ERP_ELFEC.CATEG_RECLASIF de la BD Oracle
 * @author drodriguez
 */
...
public class ReclasificacionCategoria extends Model {
	...
	/**
	 * Accede a la base de datos y obtiene todas las reclasificaciones correspondientes a una categoria
	 * @param idCategoria, id de la categoria origen
	 * @return Lista de ReclasificacionCategoria
	 */
	public static List<ReclasificacionCategoria> obtenerReclasifParaCategoria(String idCategoria)
	{
		return new Select()
        .from(ReclasificacionCategoria.class).where("IDCATEG_ORIG = ?",idCategoria)
        .execute();
	}
	
	public boolean esNecesarioRecategorizar(String idCategoria, int consumoFacturado)
	{
		 return (idCategoria.equals(IdCategoriaOrigen) && operadores.get(Condicion).evaluar(consumoFacturado, Valor));
	}
	
	//------------------------ OPERADORES DE CONDICION---------------------
	/**
	 * Guarda los operadores logicos para poder ejecutar la <b>Condicion</b> que se tiene como cadena
	 */
	private Hashtable<String, OperadorLogico> operadores = new Hashtable<String, ReclasificacionCategoria.OperadorLogico>();
	/**
	 * Pone los operadores y sus respectivas clases del tipo OperadorLogico, con la Condicion como llave
	 */
	...
	/**
	 * Provee una interfaz de operadores lógicos para ser usada en el método <b>esNecesarioRecategorizar</b>
	 * @author drodriguez
	 *
	 */
