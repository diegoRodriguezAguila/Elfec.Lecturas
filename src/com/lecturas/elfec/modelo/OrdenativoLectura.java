 ...
 * la tabla ERP_ELFEC.SGC_MOVIL_ORDENATIVOS de la base de datos ORACLE
 * @author drodriguez
 */
@Table(name = "OrdenativosLectura")
public class OrdenativoLectura extends Model implements EventoAlObtenerResultado, IModeloBackupableTexto{
	...
	/**
	 * Cuando fue enviado por 3g tiene el estado 1, 0 en caso contrario ,
	 * 2 en caso de que se lo haya enviado por cualquier otro medio
	 */
	...
	/**
	 * Accede a la base de datos y obtiene todos los ordenativos de lecturas , que tienen el atributo Enviado3G=0
	 * @return Todos los ordenativos asignados a lecturas que no se enviaron por 3g
	 */
	public static List<OrdenativoLectura> obtenerLecturasConOrdenativosNoEnviados3G()
	{

		return new Select()
        .from(OrdenativoLectura.class).where("Enviado3G=0")
        .execute();
	}
	
	/**
	 * Accede a la base de datos y obtiene las lecturas con ordenativos, ordenados por orden de cuenta de la lectura
	 * @return las lecturas que tienen los ordenativos definidos en las variables de entorno, parametrizable
	 */
	public static List<OrdenativoLectura> obtenerLecturasConOrdenativosParametros()
	{
		...
		return new Select()
        .from(OrdenativoLectura.class).where("Ordenativo IN (SELECT ID FROM Ordenativos O WHERE "+ordenativos+")")
        .execute();
	}
	...
	/**
	 * Guarda el ordenativo en la base de datos sqlite e intenta enviarlo por 3G, tambien lo guarda
	 * en el archivo de texto de backup
	...
	public String obtenerLineaTextoBackup() 
	{
		...
		str.append(Lectura.Ruta).append("|").append(Lectura.Anio).append("|").append(Lectura.Mes).append("|").append(Lectura.Dia).append("|").append(Lectura.Suministro).append("|")
		.append(Lectura.Cuenta).append("|").append(Lectura.NumeroMedidor).append("|").append(Ordenativo.Codigo).append("|")
		.append(fechaHora).append("|").append(UsuarioAuditoria);
		return str.toString();
	}
