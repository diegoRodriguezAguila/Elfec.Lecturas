...
import com.lecturas.elfec.helpers.conexionremota.EventoAlObtenerResultado;
import com.lecturas.elfec.helpers.estadosmanejadorubicacion.EventoAlObtenerUbicacion;
import com.lecturas.elfec.modelo.backuptexto.IModeloBackupableTexto;
import com.lecturas.elfec.modelo.estadoslectura.*;
import com.lecturas.elfec.modelo.validaciones.AdvConsumoBajo;
import com.lecturas.elfec.modelo.validaciones.AdvConsumoElevado;
import com.lecturas.elfec.modelo.validaciones.AdvIndiceMenorActual;
import com.lecturas.elfec.modelo.validaciones.IValidacionLectura;
import com.lecturas.elfec.modelo.validaciones.ValidacionLecturaCorrecta;
...
/**
 * Almacena la información sobre las lecturas, de la tabla Lecturas de la BD Moviles de Oracle (MOVILES.LECTURAS)
 * @author drodriguez
 *
 */
@Table(name = "Lecturas")
public class Lectura extends Model implements EventoAlObtenerUbicacion, EventoAlObtenerResultado, IModeloBackupableTexto {
    ...
    {
    	return (EvolucionConsumoLectura==null?
    			EvolucionConsumo.obtenerEvolucionConsumo(Suministro, Mes, Anio)
	...
	/**
	 * Cuando fue enviado por 3g tiene el estado 1, 0 en caso contrario , 2 si es que se envió al servidor
	 * por otro medio
	 */
	...
	/**
	 * Accede a la base de datos y obtiene todas las lecturas en orden por cuentas
	 * @return Lista de Lecturas en orden de cuentas
	 */
	public static List<Lectura> obtenerTodasLasLecturas()
	{
		return new Select()
        .from(Lectura.class).orderBy("LEMCTAANT")
        .execute();
	}
	...
	/**
	 * Llama al método de EstadoLectura, para mostrarla en una vista del tipo TomarLectura
	 * @param tomarLectura
	 */
	...
	/**
	 * Llama al método de EstadoLectura, para mostrar los menus en una vista del tipo TomarLectura
	 * @param tomarLectura
	 */
	...
	/**
	 * Asigna la implementación de la interfaz EstadoLectura, correcta según el int de estado que se
	 * le haya pasado, utiliza el factory para esto
	 * @param estado
	 */
	...
	/**
	 * Retorna la implementación de la interfaz EstadoLectura, correspondiente al entero Estado,
	 * adicionalmente verifica si el entero esta sincronizado con la clase de estado.
	...
	/**
	 * 
	 * Accede a la base de datos y obtiene todas las lecturas de la <b>ruta</b> en orden por cuentas
	 * @param ruta
	 * @return Lista de Lecturas en orden de cuentas
	 */
	...
	{
		return new Select()
        .from(Lectura.class).where("LEMRUT=?",ruta).orderBy("LEMCTAANT")
        .execute();
	}
	
	/**
	 * 
	 * Accede a la base de datos y obtiene todas las lecturas realizadas (No pendientes ni postergadas)
	 * de la <b>ruta</b> en orden por cuentas
	 * @param ruta
	...
	public static List<Lectura> obtenerLecturasRealizadasDeRuta(int ruta)
	{
		return new Select()
		.from(Lectura.class)
		.where("LEMRUT="+ruta+" AND EstadoLectura<>0 AND EstadoLectura<>3")
		.execute();
	}
	
	/**
	 * Accede a la base de datos y obtiene todas las lecturas que tengan el <b>estado</b> en orden por cuentas
	 * @param estado
	 * @return Lista de Lecturas en orden de cuentas
	 */
	...
	{
		return new Select()
        .from(Lectura.class).where("EstadoLectura=?",estado).orderBy("LEMCTAANT")
        .execute();
	}
	
	/**
	 * Accede a la base de datos y obtiene todas las lecturas que tengan el <b>estado</b> y la <b>ruta</b> en orden por cuentas
	 * @param estado
	 * @return Lista de Lecturas en orden de cuentas
	 */
	public static List<Lectura> obtenerLecturasPorEstadoYRuta(int estado, int ruta)
	{
		return new Select()
        .from(Lectura.class).where("EstadoLectura="+estado+" AND LEMRUT="+ruta).orderBy("LEMCTAANT")
        .execute();
	}
	
	/**
	 * Busca una Lectura por los ultimos digitos de la cuenta, la cuenta de busqueda debe estar
	 * en formato 00500 (sin guiones). debe ser de 5 digitos, caso contrario la busqueda retornará resultados
	 * inesperados
	 * @param cuenta
	 * @return Lista de lecturas cuyas cuentas terminan en los digitos seleccionados
	 */
	public static List<Lectura> buscarPorUltimosDigitosCuenta(String ultimosDigitosCuenta)
	{
		return new Select()
        .from(Lectura.class).where("LEMCTAANT LIKE '%"+ultimosDigitosCuenta+"'")
        .execute();
	}
	
	/**
	 * Busca una Lectura por su numero de cuenta, la cuenta de busqueda debe estar
	 * en formato 111100500 (sin guiones)
	 * @param cuenta
	 * @return Lectura correspondiente a la cuenta, o null en caso de no encontrarse
	 */
	public static Lectura buscarPorCuenta(String cuenta)
	{
		return new Select()
        .from(Lectura.class).where("LEMCTAANT=?",cuenta)
        .executeSingle();
	}
	 ...
	 * @param cuenta
	 * @return Lectura correspondiente al numero de medidor, o null en caso de no encontrarse
	 */
	public static Lectura buscarPorNumeroMedidor(String numMedidor)
	{
		return new Select()
        .from(Lectura.class).where("LEMNROMED=?",numMedidor)
        .executeSingle();
	}
	...
	{
		return new Select()
        .from(Lectura.class).where("LEMSUM=?",nus)
        .executeSingle();
	}
	
	/**
	 * Verifica si el resultado de la resta de la lectura nueva menos la anterior es elevado.
	 * @return true, false
	 */
	...
	/**
	 * Accede a la base de datos y obtiene todos los ordenativos asignados a la lectura
	...
	/**
	 * Accede a la base de datos y obtiene todos los ordenativos que aun no fueron asignados a la lectura
	 * @return lista de ordenativos
	 */
	public List<Ordenativo> obtenerOrdenativosNoUsadosLectura() {
		return new Select()
        .from(Ordenativo.class).where("TIPO_NOV<>'IMPEDIMENTO' AND TIPO_NOV<>'AUTOMATICA' AND ID NOT IN (SELECT Ordenativo FROM OrdenativosLectura WHERE Lectura="+this.getId()+")").orderBy("IDNOVEDAD")
        .execute();
    }
	...
	/**
	 * Accede a la base de datos y obtiene los conceptos asignados a la lectura, en orden de impresión
	 * @return lista de conceptos
	 */
	public List<ConceptoLectura> obtenerConceptosLecturaOrdenImpresion() {
        return new Select()
        .from(ConceptoLectura.class).where("LEMSUM=?",Suministro).where("LEMMES=?",Mes).where("LEMANO=?",Anio)
        .orderBy("OrdenImpresion")
        .execute();
    }

	/**
	 * Asigna los valores de la lecturaNueva, asi como la fecha de la lectura y 
	 * cambia su estado a <b>Leida</b>, asigna el consumo, no realiza el guardado en base de datos
	 * asigna el usuario de VariablesDeSesion como usuario de auditoria que realiza la lectura.
	...
	public void leerLectura(int lecturaNueva, Date fechaLec) {
		...
		{
			lecAnteriorCalculable = (-1)*(TopeMedidor.intValue()-LecturaAnterior);
		}
		Consumo = ((new BigDecimal(LecturaNueva - lecAnteriorCalculable)).multiply(FactorMultiplicador))
	...
	/**
	 * Obtiene el valor de la estimación de la lectura, no guarda ni cambia ningun estado 
	...
	/**
	 * Obtiene el valor de la estimación de la demanda, no guarda ni cambia ningun estado.
	...
	{
		return (new BigDecimal(ConsumoFacturado).divide((FactorCarga.multiply(new BigDecimal(HoraMesPotencia))),10, RoundingMode.HALF_UP)).setScale(0,RoundingMode.HALF_EVEN);//cambiar esto en caso de que se cambie la forma de estimacion
	}
	
	/**
	 * A partir del valor del consumo facturado, busca en la tabla de recategorizaciones si es que hay
	 * alguna recategorizacion para su categoria, y en caso de cumplir las condiciones de recategorización, la recategoriza
	 * llena los valores de <b>CategoriaRecategorizada</b> con la nueva categoria y pone <b>TagRecategorizacion</b> en 1.
	 * y se guarda con la nueva categoria, llamar despues de haber llamado a <b>leerLectura</b>
	 * @return retorna true, si es que se realizó una recategorización, false en caso contrario
	 */
	public boolean recategorizarSiEsNecesario()
	{
		List<ReclasificacionCategoria> reclasifsCat = ReclasificacionCategoria.obtenerReclasifParaCategoria(Categoria);
	...
	/**
	 * Elimina los conceptos generados para el aviso de cobranza, los Ordenativos y los datos de Consumo, ConsumoFacturado, 
	 * ImporteTap, ImporteAseo, ImporteCargoFijo, ImportePorEnergia, ImportePorPotencia, ImporteDescuentoTarifaDignidad, ImporteLey1886, NumFotosTomadas, Enviado3G, GPSLongitud, GPSLatitud
	 * LecturaNueva, FechaLecturaActual, HoraLectura y en caso de tener, de la PotenciaFacturada , FechaLecturaPotencia, HoraLecturaPotencia, EnergiaReactiva y Demanda.
	 * Tambien cambia el estado de la lectura a <b>Pendiente</b> , todos estos cambios se guardan en la base de datos
	...
	/**
	 * Verifica si aun se pueden agregar ordenativos a la lectura,
	 * dependiendo del numero maximo de ordenativos asignado en las variables de entorno
	 */
	...
	/**
	 * Elimina los conceptos de la lectura que se crearon al momento de generar el aviso de cobranza, es decir, 
	 * los que tienen el campo EsEliminable=1
	 */
	...
	/**
	 * Verifica si la lectura tiene un recordatorio, si el Recordatorio es null, vacio o esta compuesto de puros espacios
	 * devuelve false
	 * @return
	 */
	public boolean tieneRecordatorio()
	{
		return !(Recordatorio==null || Recordatorio.isEmpty() 
	    		|| contarEspacios(Recordatorio)==Recordatorio.length());
	}
	
	/**
	 * Cuenta los espacios de una cadena es usado por la funcion <b>tieneRecordatorio</b>
	 * @param cadena
	 * @return
	 */
	...
	/**
	 * Envia la lectura a guardarse remotamente y en caso de que no estuviera lista, programa la tarea para reintentarse en 40 segundos
	 */
	...
	/**
	 * Obtiene todas las lecturas realizadas (No pendientes ni postergadas), que tienen el atributo Enviado3G=0
	 * @return lista de lecturas que no se enviaron por 3g
	 */
	public static List<Lectura> obtenerLecturasNoEnviadas3G()
	{
		return new Select().from(Lectura.class).where("Enviado3G=0 AND EstadoLectura<>0 AND EstadoLectura<>3").orderBy("LEMCTAANT").execute();
	}
	
	/**
	 * Verifica que todas las lecturas se hayan realizado, y no quede ninguna pendiente ni postergada
	 * @return true, si no existe ninguna lectura pendiente ni postergada
	 */
	public static boolean seRealizaronTodasLasLecturas()
	{
		return (GestionadorBDSQLite.datosDiariosFueronCargados() && (new Select().from(Lectura.class).where("EstadoLectura=0 OR EstadoLectura=3").execute()).size()==0);
	}
	...
	public String obtenerLineaTextoBackup() 
	{
		...
		String fechaHora = df.format(this.FechaLecturaActual)+" "+this.HoraLectura;
		StringBuilder str = new StringBuilder("");
		str.append(Ruta).append("|").append(Anio).append("|").append(Mes).append("|").append(Dia).append("|").append(Suministro).append("|")
		.append(Cuenta).append("|").append(NumeroMedidor).append("|").append(LecturaNueva).append("|")
		.append(PotenciaLectura!=null?PotenciaLectura.Reactiva:"-").append("|").append(PotenciaLectura!=null?PotenciaLectura.LecturaNuevaPotencia:"-").append("|")
		.append(fechaHora).append("|").append(Estado).append("|").append(UsuarioAuditoria);
		return str.toString();
	}
