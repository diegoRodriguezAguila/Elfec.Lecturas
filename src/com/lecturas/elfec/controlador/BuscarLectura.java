...
import com.lecturas.elfec.controlador.dialogos.DialogoResultadosBusqueda;
...
public class BuscarLectura extends Activity  {
	...
	/**
	 * Pone los text listeners a la cuenta para que se pueda poner la mascara al textbox
	 */
	...
	public void btnBuscarClick(View view)
	{
		...
		if(txtCuenta.getText().length()==6)
		{
			...
			{
				lecturaEnc = Lectura.buscarPorCuenta(transformarCuenta(rutaCuenta+"-"+txtCuenta.getText()));
			}
			else
			{
				StringBuilder uDC = new StringBuilder(txtCuenta.getText().toString());
				String ultimosDigitosCuenta = uDC.delete(3, 4).toString();
				lecturasEncontradas = Lectura.buscarPorUltimosDigitosCuenta(ultimosDigitosCuenta);
			}
		}
		if(lecturaEnc==null && lecturasEncontradas.size()==0 && txtMedidor.getText().length()>4)
		{
			lecturaEnc = Lectura.buscarPorNumeroMedidor(txtMedidor.getText().toString());
		}
		if(lecturaEnc==null && lecturasEncontradas.size()==0 && txtNUS.getText().length()>4)
		{
			lecturaEnc = Lectura.buscarPorNUS(Long.parseLong(txtNUS.getText().toString()));
		}
	...
	private void procesarResultadosBusqueda(List<Lectura> lecturasEncontradas,
		...
		if(lecturasEncontradas.size()>0)
		{
			...
			{
				DialogoResultadosBusqueda dialog = new DialogoResultadosBusqueda(this, lecturasEncontradas);
				dialog.show();
			}
		}
		else
	...
	/**
	 * Muestra un dialogo que indica al usuario que no se encontraron
	...
	/**
	 * Vuelve a la actividad de Tomar lecturas y se muestra la lectura encontrada
	 * @param lecturaEnc
	 */
	public void irALecturaEncontrada(Lectura lecturaEnc) {
		Intent intent = new Intent(this, TomarLectura.class);
		intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		ManejadorDeIndice.setIdLecturaActual(lecturaEnc.getId());
