...
/**
 * Se encarga de la conexión con la base de datos oracle, y la importación y exportación de ésta.
 * @author drodriguez
 *
 */
public class ConectorBDOracle {
	...
	/**
	 * Crea una nueva conexion con la base de datos Oracle, usando la información de conexión del archivo JSON de configuración
	 * que se encuentra en la carpeta <b>assets</b> o con la configuración configurada desde el dispositivo
	 * @param contexto, la actividad de la que se llama el conector, necesaria para poder acceder a la carpeta <b>assets</b>
	 * @param habilitarRol, indica si se debe habilitar o no el rol de lecturas para esta conexión
	 */
	...
	/**
	 * Hbailita el rol de LECTURAS para el usuario logeado con el que se realiza la conexión
	 * este metodo se llama con el parametro del constructor y deberia activarse para cualquier 
	 * conexion a la base de datos oracle que requiera acceso a las tablas específicas de lecturas
	 * @param contexto, la actividad de la que se llama el conector, necesaria para poder acceder a la carpeta <b>assets</b>
	 ...
	 * OrdenativoLectura, ConceptoLectura, EvolucionConsumo y MedidorEntreLinea. También se encarga de actualizar la tabla
	 ...
	 * Toma en cuenta el dia, mes y año para realizar la importación y lo realiza de la tabla
	 * MOVILES.LECTURAS
	 * @param ruta
	 * @return
	 * @throws SQLException
	...
	private void exportarLecturas() throws SQLException
	{
		...
		for(Lectura lec : lecturas)
		{
			...
			stmt.executeUpdate(query);
			lec.Enviado3G = 2;//2 se envió al servidor, no necesariamente por 3G
			lec.save();
		}
	}
	...
	/**
	 * Se conecta a la base de datos oracle y obtiene los tokens para la conexión al servicio web 
	...
	/**
	 * Se conecta a la base de datos oracle y verifica si el imei propocionado esta aurorizado para usar la aplicación
	 * para ello accede a la tabla MOVILES.IMEI_APP y ve que exista ese IMEI y que tenga estado 1 es decir que esté activo
	 * @param imei
	 * @return
	 * @throws SQLException
	 */
	...
	/**
	 * Realiza la importación de las rutas asignadas para el usuario en la fecha actual y actualiza el estado
