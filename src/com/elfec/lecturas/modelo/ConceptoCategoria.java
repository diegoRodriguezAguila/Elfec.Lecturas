...
/**
 * Almacena la información sobre las categorias de conceptos, de la tabla ERP_ELFEC.CPTOS_CATEGORIAS de la BD Oracle
 * @author drodriguez
 */
...
public class ConceptoCategoria extends Model {
	 ...
	 * @param categoria
	 * @return Lista de ConceptoCategorias en orden de ORDEN_CALCULO, IDCONCEPTO, LIM_VALOR, IDSUBCONCEPTO
	 */
	public static List<ConceptoCategoria> obtenerConceptoCategoriasPorCategoria(String categoria)
	{
		return new Select()
        .from(ConceptoCategoria.class).where("IDCATEGORIA=?",categoria).orderBy("ORDEN_CALCULO, IDCONCEPTO, LIM_VALOR, IDSUBCONCEPTO")
        .execute();
	}
	
	/**
	 * Accede a la base de datos y obtiene todos los conceptos por categoria y aplicabilidad
	 * @param categoria
	 * @return Lista de ConceptoCategorias en orden de ORDEN_CALCULO, IDCONCEPTO, LIM_VALOR, IDSUBCONCEPTO
	 */
	public static List<ConceptoCategoria> obtenerConceptoCategoriasPorCategoriaYAplicabilidad(String categoria, String aplicabilidad)
	{
		return new Select()
        .from(ConceptoCategoria.class).where("IDCATEGORIA='"+categoria+
        		"' AND APLICABILIDAD='"+aplicabilidad+"'")
