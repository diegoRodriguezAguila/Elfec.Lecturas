...
/**
 * Almacena la información sobre la evolución de consumos, de la tabla MOVILES.LECTURASC de la BD Moviles de Oracle
 * @author drodriguez
 */
...
public class EvolucionConsumo extends Model {
	...
	/**
	 * Accede a la base de datos y obtiene la evolucion de consumo que coincida con el numero de suministro, mes y anio
	 * @param suministro
	 * @param mes
	 * @param anio
	 * @return La evolucion de consumo que coincida con esos parámetros, null si no se encuentra
	 */
	public static EvolucionConsumo obtenerEvolucionConsumo(long suministro, int mes, int anio)
	{
		return new Select()
        .from(EvolucionConsumo.class).where("LEMSUM=?",suministro).where("LEMMES=?",mes).where("LEMANO=?",anio)
        .executeSingle();
	}
	/**
	...
	{
		return Math.max(ConsumoKWH01, 
				Math.max(ConsumoKWH02, 
				 Math.max(ConsumoKWH03, 
				  Math.max(ConsumoKWH04, 
				   Math.max(ConsumoKWH05, 
				    Math.max(ConsumoKWH06, 
				     Math.max(ConsumoKWH07, 
				      Math.max(ConsumoKWH08, 
				       Math.max(ConsumoKWH09, 
				        Math.max(ConsumoKWH10, 
				         Math.max(ConsumoKWH11,ConsumoKWH12)))))))))));
	}
