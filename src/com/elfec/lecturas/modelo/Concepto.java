...
/**
 * Almacena la información sobre conceptos, de la tabla ERP_ELFEC.CONCEPTOS de la BD Oracle
 * @author drodriguez
 */
...
public class Concepto extends Model {
	...
	/**
	 * Accede a la base de datos y obtiene el concepto correspondiente al IDCONCEPTO y IDSUBCONCEPTO proporcionados
	 * @param idConcepto
	 * @param idSubConcepto
	...
	public static Concepto obtenerConcepto(int idConcepto, int idSubConcepto)
	{
		if(idConcepto>=12000)
			idConcepto-=2000;
		return new Select()
        .from(Concepto.class).where("IDCONCEPTO="+idConcepto+" AND IDSUBCONCEPTO="+idSubConcepto)
        .executeSingle();
	}
