 ...
 * ERP_ELFEC.SGC_MOVIL_PARAM de la base de datos Oracle
 * 
 * @author drodriguez
 *
 */
public class VariablesDeEntorno {
	...
	/**
	 * El protocolo que se usara para conectar con el servicio web (ej. http |
	 * https)
	 */
	public static String protocoloServicioWeb;
	/**
	 * La ip o dominio del servidor del servicio web (ej. 192.168.30.44 |
	 * webservice.elfec.com)
	 */
	...
	/**
	 * Indica el nivel de bateria considerado como crítico, cuando el nivel de
	 * bateria es igual o por debajo del crítico no se obliga al usuario a tener
	...
	/**
	 * Indica que tipo de guardado de ubicación se utilizará, para mas
	 * información vea los estados de ubicacion <b>0=no guarda, 1=guarda 3g y
	...
	/**
	 * Es un acceso rapido a la restriccion de limite de impresiones en el
	 * Administrador de seguridad, notese que este valor debe mantenerse
	 * actualizado para reflejar la sesion actual
	 */
	public static int limiteImpresiones;
	/**
	 * Es un acceso rapido a la restriccion de limite de modificaciones de una
	 * lectura en el Administrador de seguridad, notese que este valor debe
	 * mantenerse actualizado para reflejar la sesion actual
	 */
	...
	/**
	 * Inicialización simbólica que obliga a llamar al constructor estático
	 */
	...
	private static void asignarParametrosGenerales() {
		try {
			JSONObject params = new JSONObject(
					ManejadorJSON
							.LeerArchivoJSON(ConstantesDeEntorno.archivoParametrosGrales));
			numMaxOrdenativosPorLectura = params.getInt("NUMMAXORDXLEC");
			idConceptoBeneficioDignidad = params
					.getInt("IDCONCEPTOBENEFICIODIGNIDAD");
			limiteConsumoAplicaDignidad = params
					.getInt("LIMITECONSUMOAPLICADIGNIDAD");
			...
			descuentoLey1886 = new BigDecimal(
					params.getString("DESCUENTOLEY1886"));
			...
			areaImpresionTotalPorConsumo = params
					.getInt("AREAIMPTOTALPORCONSUMO");
			descripcionTotalPorConsumo = params
					.getString("DESCRIPCIONTOTALPORCONSUMO");
			areaImpresionTotalPorSuministro = params
					.getInt("AREAIMPTOTALPORSUMINISTRO");
			descripcionTotalPorSuministro = params
					.getString("DESCRIPCIONTOTALPORSUMINISTRO");
			idBaseCalculoCargoFijo = params.getInt("IDBASECALCULOCARGOFIJO");
			idBaseCalculoImporteEnergia = params
					.getInt("IDBASECALCULOIMPORTEENERGIA");
			idBaseCalculoImportePotencia = params
					.getInt("IDBASECALCULOIMPORTEPOTENCIA");
			maxConceptosAviso = params.getInt("MAXCONCEPTOSQUESEMUSTRANAVISO");
			tipoGuardadoUbicacion = params.getInt("TIPOGUARDADOUBICACION");
			timeoutGuardadoUbicacion = params.getInt("TIMEOUT") * 1000;// a
																		// milisegundos
			prefijoImpresorasRW420 = params.getString("PREFIJOIMPRESORASRW420");
			prefijoImpresorasiMZ320 = params
					.getString("PREFIJOIMPRESORASIMZ320");
			observacionesORecomendaciones = params
	...
	private static void asignarParametrosCodigosOrdenativosParaResumen() {
		try {
			...
			JSONArray params = new JSONArray(
					ManejadorJSON
							.LeerArchivoJSON(ConstantesDeEntorno.archivoCodsResumenOrdenativos));
			int num = params.length();
			for (int i = 0; i < num; i++) {
				codigosOrdenativosResumen.add(params.getJSONObject(i).getInt(
						"CODIGO"));
			}
		} catch (Exception e) {
			Log.d(e.getClass().getName(), e.getMessage());
	...
	private static void asignarParametrosCategoriasNoMostrarCargoFijo() {
		try {
			...
			JSONArray params = new JSONArray(
					ManejadorJSON
							.LeerArchivoJSON(ConstantesDeEntorno.archivoCategsNoMostrarCargoFijo));
			int num = params.length();
			for (int i = 0; i < num; i++) {
				categoriasNoCargoFijo.add(params.getJSONObject(i).getString(
						"CATEGORIA"));
			}
		} catch (Exception e) {
			Log.d(e.getClass().getName(), e.getMessage());
