 ...
 *
 */
public class DialogoPotencia extends CustomDialog{
	...
	public DialogoPotencia(Context context, Lectura lectura, boolean modoSoloLectura) {
	...
	protected void onCreate(android.os.Bundle savedInstanceState)
	{
        ...
        if(modoSoloLectura)
        {
        	...
        	RelativeLayout.LayoutParams lp = (LayoutParams) btnAceptar.getLayoutParams();
        	lp.leftMargin = 65;
			...
			public void onClick(View v) {
        	   ...
        	   try
        	   {
            	   if(lecturaActual.LeePotencia==1)//si lee reactiva
	       			{
            		   lecturaNueva = new BigDecimal(txtDemanda.getText().toString());
	       			}
            	   if(lecturaActual.LeeReactiva==1)//si lee reactiva
	       			{
            		   eReactiva = new BigDecimal(txtReactiva.getText().toString());
	       			}
        	   }
        	   ...
        	   if(seGuarda)
        	   {
    			   potencia.leerPotencia(lecturaNueva, eReactiva, lecturaActual);
        		   potencia.save();
	...
	public void mostrarOcultarReactivaYDemanda()
	{
		...
		lblPotencia.setText(""+lecturaActual.PotenciaLectura.LecturaAnteriorPotencia);
		...
		else
		{
			...
			if(modoSoloLectura)
			{
				...
				lblValDemanda.setText(lecturaActual.PotenciaLectura.LecturaNuevaPotencia.toString());
				txtDemanda.setVisibility(View.GONE);
			}
			else
			{
		...
		else
		{
			...
			if(modoSoloLectura)
			{
				...
				lblValReactiva.setText(lecturaActual.PotenciaLectura.Reactiva.toString());
				txtReactiva.setVisibility(View.GONE);
			}
			else
			{
