...
public class DialogoMedidorEntreLineas extends CustomDialog {

	/**
	 * La lectura entre lineas del dialogo, null en caso de que no se haya guardado nada
	 */
    ...
    protected void onCreate(android.os.Bundle savedInstanceState) 
    {
		...
		btnGuardar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				{
					dismiss();
					Toast.makeText(context, R.string.medidor_entre_lineas_guardado_msg, Toast.LENGTH_LONG).show();
				}
			}			
		});
		
		chkConPotencia.setOnCheckedChangeListener(new OnCheckedChangeListener() {
	...
	private boolean guardarMedidorEntreLineas() 
	{
		...
		int lecturaNueva = (txtLectura.getText().toString().equals("")?-1:Integer.parseInt(txtLectura.getText().toString()));
		BigDecimal lecturaPotencia = txtDemanda.getText().toString().equals("")?null:new BigDecimal(txtDemanda.getText().toString());
		BigDecimal energiaReactiva = txtReactiva.getText().toString().equals("")?null:new BigDecimal(txtReactiva.getText().toString());
		if(!numMedidor.isEmpty() && lecturaNueva!=-1)
		{
			medEntreLineas = new MedidorEntreLineas(numMedidor, ruta, lecturaNueva, lecturaPotencia, energiaReactiva, new Date());
			medEntreLineas.save();
			ManejadorBackupTexto.guardarBackupModelo(medEntreLineas);
			if(VariablesDeEntorno.tipoGuardadoUbicacion==0)//no se guarda ubicacion
			{
				ManejadorConexionRemota.guardarLecturaEntreLineas(medEntreLineas);
			}
			ManejadorUbicacion.obtenerUbicacionActual(context, medEntreLineas,1);// guarda ubicacion por 3g, o por gps si es que no hubiera conexion
			return true;
		}
		return false;
	}
