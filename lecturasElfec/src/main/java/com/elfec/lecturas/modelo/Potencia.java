...
public class Potencia extends Model {
	...
	/**
	 * Accede a la base de datos y obtiene la potencia que coincida con el
	 * numero de suministro, mes y anio
	 * 
	 * @param suministro
	 * @param mes
	 * @param anio
	 ...
	 * 
	 * @param lecturaPotNueva
	 *            , el valor del medidor de la nueva lectura
	 * @param reactiva
	 *            , valor de la energia reactiva, puede ser nulo
	 * @param lectura
	 *            , la lectura a la que pertenece la potencia;
	 */
	public void leerPotencia(BigDecimal lecturaPotNueva, BigDecimal reactiva,
			Lectura lectura) {
		...
		Demanda = ((lecturaPotNueva == null) ? 0 : ((lecturaPotNueva
				.multiply(FactorMultiplicadorPotencia)).setScale(0,
				RoundingMode.HALF_EVEN).intValue()));
		ConsumoFacturado = Math.max(LecturaAnteriorPotencia, Demanda);
	}

	/**
	 * Elimina los datos de , ConsumoFacturado, FechaLecturaPotencia,
	 * HoraLecturaPotencia, Demanda. todos estos cambios no se guardan en la
	 * base de datos, es necesario llamar posteriormente a guardar para que los
	 * cambios sean permanentes Se utiliza para poder dejar la lectura en estado
	 * modificable.
	 */
