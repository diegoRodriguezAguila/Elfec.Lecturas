...
/**
 * Almacena la información sobre conceptos de las lecturas, hay información de
 * la tabla MOVILES.LECTURASCONCEPTOS de la BD Moviles de Oracle. Así como
 * también información de conceptos que se generan al crear el aviso de cobranza
 * de la lectura
 *
 * @author drodriguez
 */
...
public class ConceptoLectura extends Model {
    ...
    /**
     * Este atributo se utiliza para la modificación de la lectura, los
    ...
    /**
     * Los conceptos creados con este constructor tendrán la bandera
     ...
     * @param areaImpresion
     */
    public ConceptoLectura(Lectura lectura, int idBaseCalculo,
                           String descripcion, BigDecimal importe, int ordenImpresion,
                           int areaImpresion) {
        super();
        Lectura = lectura;
        Anio = lectura.Anio;
    ...
    /**
     * Accede a la base de datos y obtiene la lectura a la que pertenece el
     * concepto, que coincida con el numero de suministro, mes y anio
     *
     * @param suministro
     * @param mes
     * @param anio
    ...
    public static Lectura obtenerLectura(long suministro, int mes, int anio) {
        return new Select().from(Lectura.class).where("LEMSUM=?", suministro)
                .where("LEMMES=?", mes).where("LEMANO=?", anio).executeSingle();
    }

    /**
     * Elimina toda la información de conceptos de lecturas que pertenezcan a la
     * asignación de ruta dada
     *
     * @param asignacionRuta
     * @param nusClausulaIN
     */
    public static void eliminarConceptosLecturasDeRutaAsignada(
            AsignacionRuta asignacionRuta, String nusClausulaIN) {
        new Delete()
                .from(ConceptoLectura.class)
    ...
    /**
     * Elimina toda la información de conceptos de lecturas
     */
    public static void eliminarConceptosLecturas() {
        new Delete().from(ConceptoLectura.class).execute();
    }
